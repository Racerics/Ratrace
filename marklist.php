<?php require('header.php'); 
	...
	// builds the query
    $query = "SELECT DISTINCT s.regid AS regid,Sem,Sub1,Sub2,Sub3,Sub4,Sub5,Sub6,L1,L2,GPA,Eligibility,verified FROM capeplacementcell.studtable s LEFT JOIN capeplacementcell.marktable m ON s.regid=m.regid ";
...
?>
	...
	<?php
        echo "<h2><i class=\"glyphicon glyphicon-user\"></i>Marklist : </h2>\n"; 
		echo "\n";
	?>
        <div class="box-icon">
        </div>
...
{		
    ...
    echo "<tr>";
		...
		echo "                Edit\n"; 
		echo "            </a>\n"; 
		echo "            <a class=\"btn btn-danger\" href=\"?did='{$row['regid']}'&sem='{$row['Sem']}'&q={$query}\">\n"; 
		echo "                <i class=\"glyphicon glyphicon-trash icon-white\"></i>\n"; 
		echo "                Delete\n"; 
		echo "            </a>\n";
		echo "            <a class=\"btn btn-success\" href=\"?vid='{$row['regid']}'&sem='{$row['Sem']}'&q={$query}\">\n"; 
		echo "                <i class=\"glyphicon glyphicon-zoom-in icon-white\"></i>\n"; 
		echo "                Verify\n";
		echo "            </a>\n";
		echo "        </td>\n";
